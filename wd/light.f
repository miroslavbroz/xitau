...
c Compute the light from the EB as a summations over the surfaces.
c Note that we've only one quadrant in the memory due to axial symmetry.
c Only partially reformatted to be more readable;
c added one output quantity (Delta_F_lambda, i.e. flux towards observer).
...
c component in front has to handled first?
...
c 2nd loop
   ...
   58 RTLEFT=-1.d0
      JK=MPP-J
   59 CONTINUE
      IX=IY+JK             ! index for component-specific arrays
      IS=IX+(KOMP-1)*LLL1  ! index for common arrays for both components
  ...
  129 COSSAV=COSGAM
   ...
   27 IF(COSGAM.GE.0.d0) GOTO 26  ! invisible
...
c Wilson & Devinney (1971), Eq. (4)
c
c                  r^2 sin(theta)
c Delta F_lambda = -------------- G D R I cos(gamma)
c                    cos(beta)
c
c r, theta, beta ... spherical coordinates of the surface
c G ... gravity darkening
c D ... limb darkening
c R ... reflection
c I ... intensity
c gamma ... angle between surface normal and observer's direction

      DIF=rit*COSGAM*DARKEN*CORFAC*(CMP*SLUMP2(IX)+CMPP*SLUMP1(IX))

c save all fluxes
      Delta_F_lambda(is) = dif*delfi*delth
      Delta_phi(is) = delfi
      Delta_theta(is) = delth
...
c    of accuracy for distorted stars without those terms. See notes.
 ...
 1145 continue
  423 continue
      ...
      SOMJ=SOMJ+DIFF
      SUMJ=SUMJ+DIF
   26 CONTINUE  ! 2nd loop

      SOMJ=SOMJ*DELFI
      SUMJ=SUMJ*DELFI
      SOM=SOM+SOMJ
      SUM=SUM+SUMJ
   36 continue  ! big loop

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c the other component
...
c big loop (for the other component)
      DO 136 I=1,NP
...
c save fluxes
 ...
 1445 continue
  ...
  126 CONTINUE  ! 2nd loop

      SOMJ=SOMJ*DELFI
      SUMJ=SUMJ*DELFI
      SOM=SOM+SOMJ
      SUM=SUM+SUMJ
  136 continue  ! big loop
