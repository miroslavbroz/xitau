...
c        if (scale(1) .gt. 0.d0) then
c          call dcopy (n,scale,1,work,1)
c          call dcopy (n,scale,1,work(istptr),1)
c        else
c          call dcopy (n,scl,0,work,1)
c          call dcopy (n,scl,0,work(istptr),1)
c        end if
        do 30 i = 1,n
          iwork(i) = i
   30   continue
...
c
   ...
   50   continue
        ...
        write(*,*) '# SUBPLEX LOOP:'
        write(*,*) '# work = ', (work(i), i=1,n)
        write(*,*) '# iwork = ', (iwork(i), i=1,n)
        write(*,*) '# nsubs = ', nsubs
c
c       simplex loop
c
