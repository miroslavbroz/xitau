...
c Miroslav Broz (miroslav.broz@email.cz), Jul 30th 2015
      ...
      subroutine read_chi2(nbod, m, elmts, d_pc, pole_l, pole_b, epoch)
...
c output
      integer nbod
      real*8 m(NPLMAX), elmts(6, NPLMAX), d_pc, pole_l, pole_b, epoch
...
c internal
     ...
     :  file_ECL, file_VIS, file_CLO
      real*8 logg(NBODMAX), metal(NBODMAX), Delta_t(NBODMAX)
...
c        R_star(i) = x_param(j)
      ...
      d_pc = x_param(j)
      j = j+1
      pole_l = x_param(j)*deg
      j = j+1
      pole_b = x_param(j)*deg
