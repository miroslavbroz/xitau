...
c Modified by: Miroslav Broz (miroslav.broz@email.cz), Mar 16th 2016

      subroutine anal_energy(nbod,mass,j2rp2,j4rp4,xb,yb,zb,
     &           vxb,vyb,vzb,ke,pot,energy,eltot)
...
c THIS TERM WAS DROPPED, because we use j2rp2 = 0 (and only barycentric coordinates)
c      if(j2rp2.ne.0.0d0) then
c         call getacch_ir3(nbod,2,xh,yh,zh,ir3h,irh)
c         call obl_pot(nbod,mass,j2rp2,j4rp4,xh,yh,zh,irh,
c     &                oblpot)
c         pot = pot + oblpot
c      endif
