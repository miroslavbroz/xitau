! multipole2.f90
! Compute gravitational potential using a multipole expansion (optimized version).
! Miroslav Broz (miroslav.broz@email.cz), Nov 14th 2019

module multipole2_module
...
double precision :: caprl, absrl, cosmphi, sinmphi
...
do l = 0, npole
  ...
  do m = 0, l
    ...
    dU_dr = caprl * (-l-1)*absrl/absr * Plm_ * (Clm(l,m)*cosmphi + Slm(l,m)*sinmphi)
    dU_dtheta = caprl * absrl * dPlm_dx(l,m,costheta,Plm_)*sintheta * (Clm(l,m)*cosmphi + Slm(l,m)*sinmphi)  ! minus or NOT?
    dU_dphi = caprl * absrl * Plm_ * (-Clm(l,m)*sinmphi*m + Slm(l,m)*cosmphi*m)
