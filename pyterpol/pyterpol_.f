...
c Miroslav Broz (miroslav.broz@email.cz), Jun 23rd 2016
     ...
     :  lambda1, lambda2, absolute, nbod2)
...
c constants
      ...
      parameter (Delta_T_eff = 1.d0)
      parameter (Delta_log_g = 0.01d0)
      parameter (Delta_v_rot = 1.d0)
c internal
...
c
      ...
      j = 0
      do i = 1, nbod
        if ((abs(T_eff(i)-T_eff1(i,k)).gt.Delta_T_eff)
     :    .or.(abs(log_g(i)-log_g1(i,k)).gt.Delta_log_g)
     :    .or.(abs(v_rot(i)-v_rot1(i,k)).gt.Delta_v_rot)) then
