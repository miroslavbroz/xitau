...
p \
  "<awk '!/^ *#/ && (NF>0){ i++; } (i%2==1){ print; }' chi2_SED.dat" u ($1/nm):3 t "real solar spectrum, real transmission" w lp pt 1 lt 1,\
  "<awk '!/^ *#/ && (NF>0){ i++; } (i%2==1){ print; }' ../test_sun0_planck/chi2_SED.dat" u ($1/nm):3 t "Planck approximation" w lp pt 1 lt 7,\
  "<awk '!/^ *#/ && (NF>0){ i++; } (i%2==1){ print; }' ../test_sun1_spectrum/chi2_SED.dat" u ($1/nm):3 t "effective wavelength/bandpass" w lp pt 1 lt 2,\
